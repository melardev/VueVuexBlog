    ...
    <div class="post" v-if="article.id">
        ...
        <div class="post-comment">
            ...
            <div class="post-comment-container">
                <div v-for="(comment,index) in getArticle.comments" :key="index"
...
<script>
    ...
    import {CommentAction} from "@/store/types.actions";

    export default {
        name: 'ArticleDetails',
        props: ['slug'],
        data() {
            return {article: {}, commentData: '', image: {}}
        },
        mounted() {
            this.fetchBySlug(this.$router.currentRoute.params.slug || this.$route.fullPath.split('/')[2]);
        },
        components: {},

        computed: {
            ...mapGetters('articles', {
                getArticle: 'getSelectedArticle',
            }),
            ...mapGetters('auth', {
        ...
        methods: {
            ...mapActions('comments', {
                createComment: CommentAction.remote.CREATE,
            }),
            fetchBySlug(slug) {
                this.$store.dispatch('articles/' + ArticleAction.remote.FETCH_BY_SLUG, slug)
                    .then(result => {
                        if (result.success) {
